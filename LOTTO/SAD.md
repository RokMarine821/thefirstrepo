...
| 1.3 | 2025-09-27 | ADR(아키텍처 의사결정 기록) 내용 보강 및 전체 문서 검토 | Gemini |
...
│  └─────────────────────────────────────────────────────────┘ │
│                            ▲                                │
│                            │ Commands(Slots/Invokables), Properties │
│                            ▼                                │
│  ┌─────────────────────────────────────────────────────────┐ │
│  │              COMMUNICATION / SERVICE PROXY LAYER        │ │
│  │ ┌─────────────┐ ┌─────────────┐ ┌─────────────────────┐ │ │
│  │ │ Backend     │ │ App         │ │ DataSync            │ │ │
│  │ │ ServiceProxy│ │ Controller  │ │ Controller          │ │ │
│  │ │ (QtRO)      │ │ (C++)       │ │ (C++)               │ │ │
│  │ └─────────────┘ └─────────────┘ └─────────────────────┘ │ │
│  │ ┌─────────────┐ ┌─────────────┐ ┌─────────────────────┐ │ │
...
│  │ │ Replicas    │ │          │ │ (NumberGenService)  │ │   │
│  │ │ (Auto-Gen)  │ │          │ │ (RuleService)       │ │   │
│  │ └──────┬──────┘ │          │ └──────────┬──────────┘ │   │
│  │        │         │          │            │            │   │
│  │        │ uses    │          │          hosts         │   │
│  │        │         │          │            │            │   │
...
public:
    ...
    /**
     * @brief 이 규칙과 관련된 과거 성과 통계를 반환합니다.
     * @param userId (선택) 특정 사용자의 통계를 조회할 경우 사용합니다.
     * @return QVariantMap 형태의 통계 데이터. RulePerformance 테이블의 레코드와 유사한 구조를 가집니다.
     *         예: {"roi": 0.15, "hitRate": 0.05, "rankDistribution": {"5": 10}}
     */
    virtual QVariantMap getStatistics(int userId = -1) const = 0;
};
```
...
**상태**: 채택됨
**날짜**: 2025-09-20
**결정**: GUI 클라이언트와 백엔드 서비스를 별도의 프로세스로 분리하는 클라이언트-서버 아키텍처를 채택한다.
**배경**:
- GUI의 반응성(UI 멈춤 현상 방지)과 백엔드의 안정적인 연산(무거운 계산)을 동시에 보장해야 한다. (NFR-001)
- 향후 웹, 모바일 등 다양한 클라이언트의 확장을 고려해야 한다. (FR-010.1)
**장점**:
- **안정성 및 반응성**: 백엔드 서비스의 오류나 과부하가 GUI 클라이언트의 UI 스레드에 직접적인 영향을 주지 않아 애플리케이션의 안정성과 사용자 경험이 향상된다.
- **독립적 배포 및 개발**: GUI와 백엔드를 독립적으로 개발, 테스트, 배포할 수 있어 개발 효율성과 유지보수성이 증대된다.
- **확장성**: 백엔드 로직의 변경 없이 새로운 클라이언트(예: 웹 대시보드)를 추가하기 용이하다.
**단점 및 완화 방안**:
- **통신 오버헤드**: 프로세스 간 통신(IPC)으로 인한 약간의 지연 시간이 발생한다. → **완화**: 로컬 통신 시 오버헤드가 적은 `QLocalSocket`(Named Pipe)을 사용하고, 데이터 직렬화/역직렬화 비용이 낮은 Qt Remote Objects를 채택하여 오버헤드를 최소화한다.
- **초기 개발 복잡성 증가**: IPC 설정, 데이터 모델 동기화 등 초기 개발 복잡성이 단일 프로세스 애플리케이션에 비해 높다. → **완화**: Qt Remote Objects의 자동 코드 생성 기능을 활용하여 통신 계층의 구현을 단순화하고, 명확한 API 인터페이스를 정의하여 복잡성을 관리한다.
