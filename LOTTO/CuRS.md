...
| 1.2 | 2025-09-20 | 요구사항 명확화 및 제약사항 통합 | Gemini |
...
- 여러 규칙을 동시에 적용 가능
- 규칙 간 우선순위 설정
- **규칙 충돌 알림**: 규칙 간 충돌(예: 동일 번호를 포함 및 제외)이 발생할 경우, 사용자에게 명확히 알리고 충돌이 해결되기 전까지 번호 생성을 방지해야 한다.
- 규칙별 가중치 설정
...
- **성과 개선 제안 (기본)**: ROI가 낮은 규칙이나 장기 미출현 번호 사용 등 통계에 기반한 간단한 개선 제안 및 권장사항을 제공해야 한다. (v1.0)
...
### FR-013: 하이브리드 GUI 구조
**우선순위**: 높음  
**설명**: QML(C++/Qt) 기반의 메인 UI와 Python(PySide6 등) 기반의 분석/AI 기능을 결합한 하이브리드 구조를 지원해야 한다.
#### FR-013.1: QML 기반 UI
- 번호 생성, 추첨 시뮬레이션, 통계 시각화 등 주요 화면은 QML(C++/Qt)로 구현한다.
- 반응형 레이아웃, 애니메이션, 접근성 등 현대적 UI/UX를 제공한다.
#### FR-013.2: Python 기반 분석 및 AI 기능
- 통계 분석, 적중률 계산, AI 추천 등 데이터 처리 및 확장 기능은 Python(PySide6 등)으로 구현한다.
- AI 모델 연동, 외부 데이터 처리, 보고서 생성 등 고급 기능을 담당한다.
#### FR-013.3: QML-Python 연동
- QML과 Python 모듈은 IPC(QProcess, JSON-RPC 등) 또는 QML-Python 바인딩을 통해 데이터를 교환한다.
- 분석 결과 및 AI 추천은 QML UI에 실시간으로 표시된다.
...
### 4.1 기술 및 아키텍처 제약사항
- **프레임워크**: QT 6.x/QML/C++ 프레임워크 사용을 필수로 한다.
- **플랫폼**: 로컬 데스크톱 애플리케이션으로 제한한다.
- **아키텍처**: 아래의 구조적 제약사항을 준수해야 한다.
  - **클라이언트-서버 분리**: GUI 클라이언트와 백엔드 서비스를 별도 프로세스로 분리해야 한다. (독립 실행 및 원격/로컬 접속 지원)
  - **플러그인 구조**: 새로운 번호 생성 규칙 등을 플러그인 형태로 추가할 수 있는 확장 가능한 구조여야 한다.
  - **데이터베이스**: 로컬 데이터 저장을 위해 SQLite를 사용한다.
  - **사용자 모드**: 초기 버전(v1.0)은 단일 사용자 모드로 개발한다.
