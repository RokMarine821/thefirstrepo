...
1.  **1단계: 규칙 기반 1차 분석**: 사람의 지식과 경험을 정형화된 '규칙'으로 만들어 로그를 신속하게 필터링하고 분류한다. (FR-03)
2.  **2단계: AI 기반 심층 분석**: 1차 분석 결과를 바탕으로 AI를 활용하여, 사람이 놓치기 쉬운 패턴이나 복잡한 상관관계를 파악하고 분석을 보강한다. (FR-04)
3.  **3단계: 실행 가능한 해결책 제시**: 2단계까지의 종합적인 분석 결과를 토대로, 실제 코드 수정이나 시스템 설정 변경에 적용할 수 있는 구체적이고 실용적인 해결책을 제안한다. (FR-05)
